...
class ResNet(nn.Module):
    ...
    def __init__(self, block, layers, num_classes=1000, k_size=[3, 3, 3, 3], plugins=None):
        ...
        if plugins is not None:
            ...
            stage_plugins = [None for i in range(4)]
        
        self.layer1 = self._make_layer(block, 64, layers[0], int(k_size[0]), plugins=stage_plugins[0])
        self.layer2 = self._make_layer(block, 128, layers[1], int(k_size[1]), stride=2, plugins=stage_plugins[1])
        self.layer3 = self._make_layer(block, 256, layers[2], int(k_size[2]), stride=2, plugins=stage_plugins[2])
        self.layer4 = self._make_layer(block, 512, layers[3], int(k_size[3]), stride=2, plugins=stage_plugins[3])
